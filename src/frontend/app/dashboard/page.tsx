              ...
              <>
                ...
                <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide font-minecraft pt-6">Selesai</h3>
                 {questHistory.completed.length === 0 ? (
                  <div className="p-4 border rounded-lg border-gray-200 bg-white text-gray-500 text-sm font-minecraft">Belum ada quest yang selesai.</div>
                ) : (
                  questHistory.completed.map((quest) => (
                    <div key={String(quest.id)} className="p-4 mt-2 border border-green-500 bg-green-50 rounded-lg opacity-80">
                      <h4 className="font-semibold text-green-800 font-minecraft flex justify-between items-center">
                        {quest.title}
                        <span className="text-xs bg-green-600 text-white px-2 py-0.5 rounded-full no-underline">Completed</span>
                      </h4>
                    </div>
                  ))
                )}
                ...
                {questHistory.failed.length === 0 ? (
                   <div className="p-4 border rounded-lg border-gray-200 bg-white text-gray-500 text-sm font-minecraft">Tidak ada quest yang gagal.</div>
                ) : (
                  questHistory.failed.map((quest) => (
                    <div key={String(quest.id)} className="p-4 mt-2 border border-red-500 bg-red-50 rounded-lg opacity-80">
                      <h4 className="font-semibold text-red-800 font-minecraft line-through flex justify-between items-center">
                        {quest.title}
                         <span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded-full no-underline">Failed</span>
                      </h4>
                    </div>
                  ))
                )}
